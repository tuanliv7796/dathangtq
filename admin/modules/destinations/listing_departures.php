...
$name_departure	= getValue("name_departure", "str", "GET", "", 1);

if($name_departure != ""){
	$sqlWhere	.= " AND dep_name LIKE '%". $name_departure . "%'";
}
...
<? /*---------Body------------*/ ?>
<?=template_top("Danh sách điểm khởi hành<a style='display: inline-block; padding-left: 50px; font-size: 15px;' href='add_departures.php'><span class='glyphicon glyphicon-plus-sign' style='font-size: 15px;' aria-hidden='true'></span>&nbsp;Thêm mới</a>")?>
<div class="listing">
	<div class="header">
		<form action="" method="GET">
			<div class="search">
				<table cellpadding="0" cellspacing="0" border="0">
					<tr>
						<td class="text">Tên địa điểm:</td>
						<td>
							<input type="text" value="<?=$name_departure?>" id="name_departure" name="name_departure" class="form-control" />
						</td>
						<td>
	...
	<div class="content">
		<table width="80%" cellspacing="0" cellpadding="0" class="table table-hover table-bordered">
				...
				while($listing = mysqli_fetch_assoc($db_listing->result)) {
					$No++;
					?>
					<tr id="tr_<?=$listing["dep_id"]?>">
						<td title="STT" class="bold center"><?=$No?></td>
						<td title="Name" class="bold center"><?=$listing["dep_name"]?></td>
						<td title="Thứ tự" class="bold center"><?=$listing["dep_order"]?></td>
						<td title="Active" class="center" style="text-align: center;">
							<a href="javascript:;" onclick="update_check_ajax('action_active', <?=$listing['dep_id']?>);" id="action_active_<?=$listing['dep_id']?>">
								<?if($listing['dep_active'] == 1){
						...
						<td  class="align_c" >
							<a href="add_departures.php?record_id=<?=$listing['dep_id']?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>" title="Bạn muốn sửa đổi bản ghi" >
