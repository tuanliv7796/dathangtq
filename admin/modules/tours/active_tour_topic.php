...
$db_check	= new db_query("	SELECT * FROM tours_topics
										WHERE toto_id = " . $record_id
									);
if($row	= mysqli_fetch_assoc($db_check->result)){
	//Kiểm tra field
	switch($type){
		case 'action_active':
			$value	= abs(1 - $row['toto_active']);
			$sql		= " toto_active = " . abs(1 - $row['toto_active']);
			break;

		case 'action_show_home':
			$value	= abs(1 - $row['toto_show_home']);
			$sql		= " toto_show_home = " . abs(1 - $row['toto_show_home']);
			break;
	}

	if($sql != "" && isset($value) && ($value == 1 || $value == 0)){
		// Cập nhật dữ liệu
		$db_update	= new db_execute("UPDATE tours_topics SET " . $sql . " WHERE toto_id = " . $record_id);
