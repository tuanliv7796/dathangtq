...
$db_check	= new db_query("	SELECT * FROM vehicles
										WHERE veh_id = " . $record_id
									);
if($row	= mysqli_fetch_assoc($db_check->result)){
	//Kiểm tra field
	switch($type){
		case 'action_active':
			$value	= abs(1 - $row['veh_active']);
			$sql		= " veh_active = " . abs(1 - $row['veh_active']);
			break;
	}

	if($sql != "" && isset($value) && ($value == 1 || $value == 0)){
		// Cập nhật dữ liệu
		$db_update	= new db_execute("UPDATE vehicles SET " . $sql . " WHERE veh_id = " . $record_id);
