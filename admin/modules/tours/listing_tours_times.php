...
$name_time	= getValue("name_time", "str", "GET", "", 1);

if($name_time != ""){
	$sqlWhere	.= " AND tot_name LIKE '%". $name_time . "%'";
}
...
<? /*---------Body------------*/ ?>
<?=template_top("Danh sách thời lượng tour<a style='display: inline-block; padding-left: 50px; font-size: 15px;' href='add_tours_times.php'><span class='glyphicon glyphicon-plus-sign' style='font-size: 15px;' aria-hidden='true'></span>&nbsp;Thêm mới</a>")?>
<div class="listing">
	<div class="header">
		<form action="" method="GET">
			<div class="search">
				<table cellpadding="0" cellspacing="0" border="0">
					<tr>
						<td class="text">Tên thời lượng tour:</td>
						<td>
							<input type="text" value="<?=$name_time?>" id="name_time" name="name_time" class="form-control" />
						</td>
						<td>
	...
	<div class="content">
		<table width="80%" cellspacing="0" cellpadding="0" class="table table-hover table-bordered">
				<tr class="warning">
					...
					<td class="h">Tên thời lượng tour</td>
				...
				while($listing = mysqli_fetch_assoc($db_listing->result)) {
					$No++;
					?>
					<tr id="tr_<?=$listing["tot_id"]?>">
						<td title="STT" class="bold center"><?=$No?></td>
						<td title="Name" class="bold center"><?=$listing["tot_name"]?></td>
						<td title="Thứ tự" class="bold center"><?=$listing["tot_order"]?></td>
						<td title="Active" class="center" style="text-align: center;">
							<a href="javascript:;" onclick="update_check_ajax('action_active', <?=$listing['tot_id']?>);" id="action_active_<?=$listing['tot_id']?>">
								<?if($listing['tot_active'] == 1){
						...
						<td  class="align_c" >
							<a href="add_tours_times.php?record_id=<?=$listing['tot_id']?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>" title="Bạn muốn sửa đổi bản ghi" >
