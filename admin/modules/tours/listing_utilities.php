...
$name_utility	= getValue("name_utility", "str", "GET", "", 1);

if($name_utility != ""){
	$sqlWhere	.= " AND uti_name LIKE '%". $name_utility . "%'";
}
...
<? /*---------Body------------*/ ?>
<?=template_top("Danh sách tiện ích<a style='display: inline-block; padding-left: 50px; font-size: 15px;' href='add_utilities.php'><span class='glyphicon glyphicon-plus-sign' style='font-size: 15px;' aria-hidden='true'></span>&nbsp;Thêm mới</a>")?>
<div class="listing">
	<div class="header">
		<form action="" method="GET">
			<div class="search">
				<table cellpadding="0" cellspacing="0" border="0">
					<tr>
						<td class="text">Tên tiện ích:</td>
						<td>
							<input type="text" value="<?=$name_utility?>" id="name_utility" name="name_utility" class="form-control" />
						</td>
						<td>
	...
	<div class="content">
		<table width="80%" cellspacing="0" cellpadding="0" class="table table-hover table-bordered">
				<tr class="warning">
					...
					<td class="h">Tên tiện ích</td>
					<td class="h">Style class</td>
				...
				while($listing = mysqli_fetch_assoc($db_listing->result)) {
					$No++;
					?>
					<tr id="tr_<?=$listing["uti_id"]?>">
						<td title="STT" class="bold center"><?=$No?></td>
						<td title="Name" class="bold center"><?=$listing["uti_name"]?></td>
						<td title="Style" class="bold center"><?=$listing["uti_class_css"]?></td>
						<td title="Thứ tự" class="bold center"><?=$listing["uti_order"]?></td>
						<td title="Active" class="center" style="text-align: center;">
							<a href="javascript:;" onclick="update_check_ajax('action_active', <?=$listing['uti_id']?>);" id="action_active_<?=$listing['uti_id']?>">
								<?if($listing['uti_active'] == 1){
						...
						<td  class="align_c" >
							<a href="add_utilities.php?record_id=<?=$listing['uti_id']?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>" title="Bạn muốn sửa đổi bản ghi" >
