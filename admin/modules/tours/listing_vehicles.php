...
$name_vehicle	= getValue("name_vehicle", "str", "GET", "", 1);

if($name_vehicle != ""){
	$sqlWhere	.= " AND veh_name LIKE '%". $name_vehicle . "%'";
}
...
<? /*---------Body------------*/ ?>
<?=template_top("Danh sách phương tiện<a style='display: inline-block; padding-left: 50px; font-size: 15px;' href='add_vehicles.php'><span class='glyphicon glyphicon-plus-sign' style='font-size: 15px;' aria-hidden='true'></span>&nbsp;Thêm mới</a>")?>
<div class="listing">
	<div class="header">
		<form action="" method="GET">
			<div class="search">
				<table cellpadding="0" cellspacing="0" border="0">
					<tr>
						<td class="text">Tên phương tiện:</td>
						<td>
							<input type="text" value="<?=$name_vehicle?>" id="name_vehicle" name="name_vehicle" class="form-control" />
						</td>
						<td>
	...
	<div class="content">
		<table width="80%" cellspacing="0" cellpadding="0" class="table table-hover table-bordered">
				...
				while($listing = mysqli_fetch_assoc($db_listing->result)) {
					$No++;
					?>
					<tr id="tr_<?=$listing["veh_id"]?>">
						<td title="STT" class="bold center"><?=$No?></td>
						<td title="Name" class="bold center"><?=$listing["veh_name"]?></td>
						<td title="Style" class="bold center"><?=$listing["veh_class_css"]?></td>
						<td title="Thứ tự" class="bold center"><?=$listing["veh_order"]?></td>
						<td title="Active" class="center" style="text-align: center;">
							<a href="javascript:;" onclick="update_check_ajax('action_active', <?=$listing['veh_id']?>);" id="action_active_<?=$listing['veh_id']?>">
								<?if($listing['veh_active'] == 1){
						...
						<td  class="align_c" >
							<a href="add_vehicles.php?record_id=<?=$listing['veh_id']?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>" title="Bạn muốn sửa đổi bản ghi" >
