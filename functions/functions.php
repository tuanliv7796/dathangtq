...
function generateBreadcrumbs($iCat = 0, $htmlAdd = ''){
	...
	$nav	= '<li itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem"><a itemscope="" itemtype="http://schema.org/Thing" itemprop="item" href="' . ROOT_PATH . '"><i class="fa fa-home"></i> Trang chủ</a><meta itemprop="position" content="1"></li>';
	if($iCat > 0){
		...
		if(count($array_parent) > 0){
			...
			foreach($array_parent as $key => $value){
				...
				$type 		= $value['cat_type'];
				if($type == 'tour') $type = "category";
				$link			= createlink($type, $arrayLink);
				$class	= "";
				if($iCat == $key) $class	= "current";
				$nav		.= '<li itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem" id="liRegion"><a itemscope="" itemtype="http://schema.org/Thing" itemprop="item" href="' . $link . '" title="' . htmlspecialbo($value['cat_name']) . '">' . $value['cat_name'] . '</a><meta itemprop="position" content="' . $stt . '"></li>';
			}
		} // End if(count($array_parent) > 0)
	}
	if($htmlAdd != ""){
		$nav 	.= '<li itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem" class="active hidden-xs">
						' . $htmlAdd . '
						<meta itemprop="position" content="' . ($stt + 1) . '">
					</li>';
	}

	$nav 	= '<ol class="breadcrumb" itemscope="" itemtype="http://schema.org/BreadcrumbList">' . $nav . '</ol>';
