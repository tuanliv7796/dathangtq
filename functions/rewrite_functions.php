...
function createlink($module = "", $row = array())
{
   ...
   switch ($module) {
      case "category":
         $strlink = ROOT_PATH . 'tour/' . removeTitle($row["nTitle"]);
         break;
      case "cat_blog":
         $strlink = ROOT_PATH . 'blog/' . removeTitle($row["cTitle"]);
         break;
      case "news_detail":
         $strlink = ROOT_PATH . 'blog/' . $row["iData"] . '/' . removeTitle($row["nTitle"]);
         break;
      case "topic":
         $strlink = ROOT_PATH . 'tour/' . removeTitle($row["nTitle"]) . '/c' . $row["iData"];
         break;
      case "tour":
         $strlink = ROOT_PATH . 'tour/' . removeTitle($row["nTitle"]) . '-id' . $row["iData"] . ".html";
         break;
      case "static_detail":
         $strlink = ROOT_PATH . 'static/' . removeTitle($row["sTitle"]);
         break;
      case "agency_detail":
         $strlink = ROOT_PATH . 'agency/' . removeTitle($row["nTitle"]).".html";
         break;
      case "news":
         if (isset($row["iData"]) && $row["iData"] > 0) {
            $strlink = ROOT_PATH . 'news/' . removeTitle($row["nTitle"]) . '-id' . $row["iData"];
         } else {
